...
void CompareBranch(string MCfilename, string REfilename, string branchname,
    ...
    Double_t xmin, xmax;
    xmin = REtree->GetMinimum(branchname.c_str());
    if (xmin < MCtree->GetMinimum(branchname.c_str())) xmin = MCtree->GetMinimum(branchname.c_str());
    xmax = REtree->GetMaximum(branchname.c_str());
    if (xmax > MCtree->GetMaximum(branchname.c_str())) xmin = MCtree->GetMaximum(branchname.c_str());
    if (xlow < xmin) {
        xlow = xmin;
    }
    if (xup > xmax) {
